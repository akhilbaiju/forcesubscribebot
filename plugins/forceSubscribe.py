    ...
    if chat_member.restricted_by:
      if chat_member.restricted_by.id == (client.get_me()).id:
          try:
            ...
            client.answer_callback_query(cb.id, text="ചേട്ടാ ഞാൻ പറഞ്ഞ ചാനലിൽ ഒന്ന് ജോയിൻ ചെയ്തിട്ട്  unmute ഞെക്. Join Channel & Press UnMute 😜", show_alert=True)
      else:
        client.answer_callback_query(cb.id, text="❗ Admins എന്തിനോ നിന്നെ mute ചെയ്തേക്കുവാ 🤔", show_alert=True)
    else:
      if not client.get_chat_member(chat_id, (client.get_me()).id).status == 'administrator':
        ...
        client.answer_callback_query(cb.id, text="നിനക്ക് മെസ്സേജ് അയക്കാൻ പറ്റുന്നുണ്ടെങ്കിൽ വെറുതെ ഇതിൽ click ചെയ്യണ്ട", show_alert=True)
        ...
        try:
          for chat_member in client.get_chat_members(message.chat.id, filter="restricted"):
            if chat_member.restricted_by.id == (client.get_me()).id:
                ...
                time.sleep(1)
          sent_message.edit('✅ **ഏല്ലാരേം ഞാൻ Free ആക്കി 😉**')
        except ChatAdminRequired:
